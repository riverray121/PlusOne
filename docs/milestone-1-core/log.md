...
- All selection editing goes through SelectionEditor. Three traps it guards against: (1) picker deselection can fail to update its binding, and a mounted picker can write a stale selection back into a live binding, so the picker works on a private buffer merged once on close; (2) FamilyActivitySelection carries member views (applications/webDomains/categories) parallel to the token sets, and a token removed without its member entry resurrects through an encode/decode round trip, so removal rebuilds a fresh token-only selection; (3) persistence never relies on Equatable-based onChange of a selection; every mutation commits explicitly.
